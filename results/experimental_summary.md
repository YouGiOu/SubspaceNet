...
33. Phase `7H` then exposed an important deployment gap. When the Phase `7G` model was evaluated on continuous random-gap test sets inside `[-15 deg, 15 deg]`, performance collapsed badly even though runtime stayed stable: coherent random-gap mean RMSE was about `5.64 deg` and non-coherent random-gap mean RMSE was about `5.09 deg`. This is strong evidence that the fixed-gap and boundary-aware training logic does not automatically transfer to continuous random-gap evaluation.
34. Phase `7H` is especially informative because the realized random-gap datasets were not concentrated only in the hardest low-gap corner; their mean gap was about `10.6 deg` with median around `9.3 - 9.6 deg`. So the failure is not simply that the random-gap test set was harder than the fixed-gap grid. The more plausible interpretation is that the model had become too specialized to the fixed-gap training structure and did not learn a strong continuous-gap representation.
35. Phase `7I` then tested the first staged random-gap fine-tuning strategy, starting from Phase `7G` and adding three sequential `45k` stages with increasing random-gap exposure. This materially improved deployment-style transfer: coherent random-gap mean RMSE improved from about `5.64 deg` to about `4.14 deg`, and non-coherent random-gap mean RMSE improved from about `5.09 deg` to about `3.52 deg`.
36. However, Phase `7I` also makes clear that simply adding more random-gap training is not yet the right balance. Compared with Phase `7G`, the coherent `1 deg`, `1 dB` anchor worsened from about `0.3911 deg` to about `0.5042 deg`, the coherent `2 deg` boundary mean worsened from about `1.13 deg` to about `1.24 deg`, and most coherent `1 - 3 deg` fixed-gap cells degraded. So the current Phase `7I` result is a genuine random-gap improvement, but it overpays by giving back too much of the hard-cell and boundary skill that made Phase `7G` valuable.
37. The Phase `7I` stage metrics also suggest why. The per-stage held-out RMSE drifted upward across the three training stages, from about `0.8999 deg` in Stage `1` to about `0.9672 deg` in Stage `2` and about `1.0185 deg` in Stage `3`, even though each stage still converged numerically. Current working interpretation: the later random-gap-heavy stages likely kept broadening the model after the best retention-versus-generalization tradeoff had already passed.
38. The current best interpretation is therefore now five-way rather than three-way: Phase `7D` remains the best pure hard-cell specialist, Phase `7F` is the strongest broad fixed-gap mixed-condition generalization step, Phase `7G` is the strongest targeted coherent `2 deg` boundary-repair variant, Phase `7H` is the key evidence that fixed-gap skill does not automatically imply random-gap readiness, and Phase `7I` is the first positive but still over-aggressive random-gap adaptation attempt.
39. Root-MUSIC remains the most fragile component in both classical and learned forms and should still be treated as experimental unless it is the explicit object of study.
40. The experiment framework is now mature enough that future work should be targeted:
   ...
   - random-gap follow-up that keeps explicit continuous-gap exposure but stops earlier or protects the structured low-gap cells more strongly, because the first Phase `7I` attempt improved random-gap transfer but over-erased the coherent `1 - 2 deg` fixed-gap strengths
