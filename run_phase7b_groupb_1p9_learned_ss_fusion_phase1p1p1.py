"""Launch Phase 7B Phase 1.1.1 Group B 1.9 lambda 1x1 SS-fusion experiment family."""
...
PHASE1P1P1_EXPERIMENTS = [
    "phase7b_gb19_coh_g1_s1_ss33_subspacenet_phase1p1p1",
    "phase7b_gb19_coh_g1_s1_ss23_r01_subspacenet_phase1p1p1",
    "phase7b_gb19_coh_g1_s1_ssfusion_spatial_subspacenet_phase1p1p1",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Run Phase 7B Phase 1.1.1 Group B 1.9 lambda 1x1 learned SS-fusion experiments "
    ...
    parser.add_argument(
        "--results-dir",
        default="results/phase7b_groupb_1p9_learned_ss_fusion_phase1p1p1",
        help="Dedicated output directory for the Phase 7B Phase 1.1.1 study.",
    )
    parser.add_argument(
        "--templates-dir",
        default="data/dataset_templates/ablation/phase7b_groupb_1p9_learned_ss_fusion_phase1p1p1",
        help="Generated template directory for the Phase 7B Phase 1.1.1 study.",
    )
    parser.add_argument(
        "--skip-baseline-reuse",
        action="store_true",
        help="Do not copy reusable Phase 1.1 baseline results into the Phase 1.1.1 results folder.",
    )
    return parser.parse_args()


def ensure_phase7b_phase1p1p1_templates(repo_root: Path, generated_templates_dir: Path):
    source_path = (
        repo_root
        / "data"
        / "dataset_templates"
    ...
    variants = [
        {
            "template_name": "phase7b_gb19_coh_g1_s1_ss33_subspacenet_phase1p1p1",
            "description": (
                "Phase 7B Phase 1.1.1 coherent Group B 1.9 lambda learned baseline for "
        ...
        {
            "template_name": "phase7b_gb19_coh_g1_s1_ss23_r01_subspacenet_phase1p1p1",
            "description": (
                "Phase 7B Phase 1.1.1 coherent Group B 1.9 lambda fixed-best-pair learned "
        ...
        {
            "template_name": "phase7b_gb19_coh_g1_s1_ssfusion_spatial_subspacenet_phase1p1p1",
            "description": (
                "Phase 7B Phase 1.1.1 coherent Group B 1.9 lambda reused spatial-fusion "
                "reference for gap=1 deg and SNR=1 dB using SS(2/3 x 3) -> LRMC -> learned "
                "fusion -> SubspaceNet -> ESPRIT."
            ),
        ...
        {
            "template_name": "phase7b_gb19_coh_g1_s1_ssfusion_1x1_subspacenet_phase1p1p1",
            "description": (
                "Phase 7B Phase 1.1.1 coherent Group B 1.9 lambda 1x1 channel-only learned "
                "SS-fusion model for gap=1 deg and SNR=1 dB using three SS(2/3) -> LRMC "
                "branches followed by 1x1 learned fusion and the SubspaceNet-ESPRIT backbone."
            ),
            "scheme_label": "Coherent | Gap = 1 deg | SNR = 1 dB | SS(2/3 x 3) -> LRMC -> 1x1 learned fusion -> SubspaceNet",
    ...
    for variant in variants:
        ...
        report["markdown_description"] = (
            "Phase 7B Phase 1.1.1 compares the new 1x1 channel-only learned SS-fusion "
            "candidate against the three learned baselines already established in the "
            "same primary coherent hard cell."
        )
...
def reuse_phase1p1_baselines(repo_root: Path, results_dir: Path):
    source_root = (
        repo_root / "results" / "phase7b_groupb_1p9_learned_ss_fusion_phase1p1"
    )
    mapping = {
...
def main():
    ...
    ensure_phase7b_phase1p1p1_templates(repo_root, templates_dir)
    if not args.skip_baseline_reuse:
        reuse_phase1p1_baselines(repo_root, results_dir)
    ...
    print(
        "Phase 7B Phase 1.1.1 experiment: Group B 1.9 lambda 1x1 learned SS-fusion "
        "primary coherent hard-cell family"
    )
    print("Results will be written under:", args.results_dir)
    if not args.skip_baseline_reuse:
        print("Phase 1.1 baselines will be reused when available.")
